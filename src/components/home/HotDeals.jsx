        ...
        <div className="grid grid-cols-2 grid-rows-3 md:grid-cols-3 lg:grid-cols-5">
          {maxProdToShow.map((item, index) => (
            ...
            {/* First Big sale product */}
              {index === 0 && (
                <div
                  className="right col-span-2 row-span-2 size-full bg-white px-6 border border-hard-primary pb-6 shadow-[0_15px_40px_rgba(32,181,38,0.12)] group  md:h-full relative"
                  key={index}
                >
                  <div className="">
                    {/* discount badge */}
                    {item.sale && (
                      <div className="py-1 px-2 rounded-sm bg-danger text-gray-0 font-poppins test-sm font-medium absolute top-3 left-2 capitalize">
                        <p>
                          sale{" "}
                          <span className="font-semibold">{item.sale}%</span>
                        </p>
                      </div>
                    )}
                  ...
                  <div className="flex flex-col max-h-full">
                    <div className="flex items-center justify-between gap-x-1 pb-8">
                      {/* wishlist */}
                      <div onClick={() => openQuickView(maxProdToShow[2].slug)} className="w-8 md:w-12 h-8 md:h-12 rounded-full flex items-center justify-center  bg-[#F2F2F2] border border-gray-100 cursor-pointer ">
